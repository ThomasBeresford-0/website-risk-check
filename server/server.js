...
app.get("/admin/generate-sample", async (req, res) => {
  try {
    if (process.env.ENABLE_SAMPLE_GEN !== "true") {
      return res.status(404).send("Not found");
    }

    const adminKey = String(req.query.admin_key || "");
    ...
    if (!nonce || nonce !== process.env.SAMPLE_NONCE) {
      return res.status(403).send("Forbidden");
    }
    if (!url) return res.status(400).send("Missing url");

    // Optional IP allowlist
    const allow = String(process.env.SAMPLE_IP_ALLOWLIST || "").trim();
    if (allow) {
      const ip =
      ...
      JSON.stringify(
        {
          ...
          createdAt: Date.now(),
          integrityHash,
          scanData: { ...scanData, integrityHash },
        },
        null,
        2
      )
    ...
    // Burn the nonce so this can’t be re-used (best effort)
    // (You’ll manually rotate/remove SAMPLE_NONCE after anyway)
    return res.json({
      ok: true,
      token,
      integrityHash,
      reportUrl: `${BASE_URL}/r/${token}`,
      verifyUrl: `${BASE_URL}/verify/${integrityHash}`,
    });
  } catch (e) {
